...
struct AIChatScreen: View {
    ...
    private func generateResponse(for userInput: String) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                // Decode JSON properly by accessing the "recommendations" key
                if let jsonResponse = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   let recommendations = jsonResponse["recommendations"] as? [[String: String]] {
                    ...
                    let formattedResponse = recommendations.map { recommendation in
                        ...
                        **\(recommendation["name"] ?? "Unknown")**
                        ...
                        """
                    }.joined(separator: "\n\n") // Adds spacing between recommendations

                    DispatchQueue.main.async {
                        messages.append(ChatMessage(id: UUID(), text: formattedResponse, isUser: false))
                    }
                } else {
                    DispatchQueue.main.async {
                        messages.append(ChatMessage(id: UUID(), text: "Invalid response format.", isUser: false))
                    }
                }
            } catch {
                DispatchQueue.main.async {
                    messages.append(ChatMessage(id: UUID(), text: "Error parsing AI response.", isUser: false))
                }
            }
        }.resume()
    }
